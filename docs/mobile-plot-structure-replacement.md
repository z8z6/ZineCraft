...
本文说明如何替换移动地块的动力层、支持层、生活层，以及各国家的普通商店和中型商店 NBT。

## 1. 当前生成方式

一个 Region 的移动地块由以下内容组成：

1. 每个 Chunk 从下到上放置动力层、支持层和生活层。
2. 三层上方放置道路或建筑。
3. 建筑模板默认正面朝南，也就是本地坐标的 +Z 方向；运行时根据临路方向自动旋转。
...
分层结构会在 Region 的每个 Chunk 重复一次，因此层级模板必须能够无缝平铺。商店则按规划阶段分配的完整 Chunk 矩形放置。
