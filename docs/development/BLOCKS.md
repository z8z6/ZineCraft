...
注册字段是 NeoForge 的延迟方块句柄，需要原版 `Block` 时调用 `get()`：

```java
BlockState state = TEST_STONE_BRICKS.get().defaultBlockState();
```

需要传入 `Supplier<? extends Block>` 或 `ItemLike` 的 API 时可直接使用 `TEST_STONE_BRICKS`。
...
目录会阻止同时设置“掉落自身”和“掉落指定物品”。修改后必须重新运行 `runData`。
...
| 放下正常但物品图标缺失  | 确认没有调用 `noBlockItem()`，再运行 `runData` |
