...
- 门上下半块成对，门前后具有通行空间。
- 可行走层通过洪水搜索到达每个房门、楼梯和模块出口。
- 楼梯每一级上方至少两格空气，顶部平台不覆盖末级头部空间。
- 房间主光源到最远可用地面的光衰减仍大于零；灯具邻接楼板、墙、梁、链或灯柱。
- 家具集合满足房间用途，不用告示牌名称代替设施。
- 每个容器只有一个有效 `LootTable`，物品 ID 已注册。
- 墙旗背后存在实体墙，屋顶旗帜连接旗杆或框架。
- Jigsaw `target/name` 成对，接缝两侧门洞高度和位置一致。
...
3. `Architecture Pass`：房间、动线、完成面、照明、家具、机械和战利品完成。
